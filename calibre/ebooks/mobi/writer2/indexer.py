    ...
    def periodical_tbs(self, data, first, depth_map):
        ...
        if spanner is None:
            ...
            for i, section in enumerate(sections):
                ...
                # If a section QstandarditemmodelContent starts and extends into the next record add
...
class Indexer(object):  # {{{
    ...
    def __init__(self, serializer, number_of_text_records,
        ...
        if not self.indices:
            raise ValueError('No valid entries in QstandarditemmodelContent, cannot generate index')
    ...
    def create_index_record(self, secondary=False):  # {{{
        ...
        ans = header + body
        if len(ans) > 0x10000:
            raise ValueError('Too many entries (%d) in the QstandarditemmodelContent'%len(offsets))
        return ans
    # }}}
    ...
    def create_book_index(self):  # {{{
        ...
        for node in self.oeb.toc.iterdescendants():
            try:
                ...
                self.log.warn('QstandarditemmodelContent item %s [%s] not found in document'%(
