...
class FB2MLizer(object):
    ...
    def get_text(self):
        ...
        for item in self.oeb_book.spine:
            ...
            # Start a <section> if we must sectionize each file or if the QstandarditemmodelContent references this page
    ...
    def dump_text(self, elem_tree, stylizer, page, tag_stack=[]):
        ...
        # Convert QstandarditemmodelContent entries to <title>s and add <section>s
        if self.opts.sectionize == 'toc':
            # A section cannot be a child of any other element than another section,
            # so leave the tag alone if there are parents
            if not tag_stack:
                # There are two reasons to start a new section here: the QstandarditemmodelContent pointed to
                # this page (then we use the first non-<body> on the page as a <title>), or
                # the QstandarditemmodelContent pointed to a specific element
                newlevel = 0
