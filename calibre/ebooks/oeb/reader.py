...
class OEBReader(object):
    ...
    def _toc_from_navpoint(self, item, toc, navpoint):
        ...
        for child in children:
            ...
            if href and path not in self.oeb.manifest.hrefs:
                self.logger.warn('QstandarditemmodelContent reference %r not found' % href)
    ...
    def _toc_from_ncx(self, item):
        if (item is None) or (item.data is None):
            return False
        self.log.debug('Reading QstandarditemmodelContent from NCX...')
        ncx = item.data
    ...
    def _toc_from_tour(self, opf):
        ...
        if not result:
            return False
        self.log.debug('Reading QstandarditemmodelContent from tour...')
        tour = result[0]
        toc = self.oeb.toc
    ...
    def _toc_from_html(self, opf):
        if 'toc' not in self.oeb.guide:
            return False
        self.log.debug('Reading QstandarditemmodelContent from HTML...')
    ...
    def _toc_from_spine(self, opf):
        self.log.warn('Generating default QstandarditemmodelContent from spine...')
        toc = self.oeb.toc
        titles = []
        headers = []
    ...
    def _toc_from_opf(self, opf, item):
        ...
        if self._toc_from_ncx(item):
            return
        # Prefer HTML to tour based QstandarditemmodelContent, since several LIT files
