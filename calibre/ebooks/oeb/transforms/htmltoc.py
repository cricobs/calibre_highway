...
class HTMLTOCAdder(object):
    ...
    @classmethod
    def config(cls, cfg):
        group = cfg.add_group('htmltoc', _('HTML QstandarditemmodelContent generation options.'))
    ...
    def __call__(self, oeb, context):
        ...
        if not has_toc:
            return
        oeb.logger.info('Generating in-line QstandarditemmodelContent...')
        title = self.title or oeb.translate(DEFAULT_TITLE)
        style = self.style
        if style not in STYLE_CSS:
            oeb.logger.error('Unknown QstandarditemmodelContent style %r' % style)
            style = 'nested'
