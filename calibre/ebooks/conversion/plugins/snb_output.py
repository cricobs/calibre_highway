...
class SNBOutput(OutputFormatPlugin):
    ...
    def convert(self, oeb_book, output_path, input_plugin, opts, log):
        ...
        with TemporaryDirectory('_snb_output') as tdir:
            ...
            # Output QstandarditemmodelContent
            ...
            if oeb_book.toc.count() == 0:
                log.warn('This SNB file has no Table of Contents. '
                    'Creating a default QstandarditemmodelContent')
            ...
            for tocitem in oeb_book.toc:
                if tocitem.href.find('#') != -1:
                    ...
                    if len(item) != 2:
                        log.error('Error in QstandarditemmodelContent item: %s' % tocitem)
                    else:
            ...
            for item in s:
                ...
                if m.hrefs[item.href].media_type in OEB_DOCS:
                    if item.href not in outputFiles:
                        log.debug('File %s is unused in QstandarditemmodelContent. Continue in last chapter' % item.href)
                        mergeLast = True
                    else:
                    ...
                    if not mergeLast:
                        ...
                        log.debug('Merge %s with last QstandarditemmodelContent item...' % item.href)
