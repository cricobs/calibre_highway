        ...
        for node in self.oeb.toc.iter():
            ...
            if hasattr(href, 'partition'):
                ...
                if frag and frag_pat.match(frag) is None:
                    self.log.warn(
                            'Removing fragment identifier %r from QstandarditemmodelContent as Adobe Digital Editions cannot handle it'%frag)
                    node.href = base
                    ...
                    for x in self.oeb.spine:
                        if x.href == href:
                            if frag_is_at_top(x.data, frag):
                                self.log.debug('Removing anchor from QstandarditemmodelContent href:',
                                        href+'#'+frag)
                                toc.href = href
                            break
            for x in toc:
