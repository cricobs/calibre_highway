...
class CoversView(QListView):  # {{{
    ...
    def show_cover(self):
        ...
        if pmap is None and idx.row() == 0:
            pmap = self.model().cc
        if pmap is not None:
            from calibre.gui2.viewer.qdialog.qdialogImage import QdialogImage
            d = QdialogImage(self, pmap, unicode(idx.data(Qt.DisplayRole) or ''), geom_name='metadata_download_cover_popup_geom')
            d(use_exec=True)
